...
class ExportForm extends \yii\base\Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['filename'], 'string'],
            [['filename'], 'trim'],
            [['content', 'exportColumnsName', 'excelIgnoreAttributesName', 'tableColumnsName'], 'safe'],
        ];
    }
    ...
    public function getExportColumns($searchModel, $type='excel')
    {
        ...
        if ($type == 'excel' && (($excelIgnoreAttributes = $searchModel->{$this->excelIgnoreAttributesName}) != null) ) {
            $ignoreAttributes = array_merge($ignoreAttributes, $excelIgnoreAttributes);
        }

        $tableColumns = array_filter($searchModel->{$this->tableColumnsName},
