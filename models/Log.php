...
class Log extends ActiveRecord
{
    ...
    public static function record($model, $changedAttributes=[])
    {
        // if (App::isLogin()) {
           ...
           if ($log->controller!='site' && $log->save()) {
                
               $range_interval=date("Y-m-d", strtotime("-1 month", strtotime(date("Y-m-d H:i:s"))  )); // delete old records logs
	    	   // Log::deleteAll("date(created_at)<'".$range_interval."' ");
	    	   $connection=Yii::$app->db;  
               $query="delete from {{%logs}} where date(created_at)<'".$range_interval."'  ";  
