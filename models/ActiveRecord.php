...
abstract class ActiveRecord extends \yii\db\ActiveRecord
{
    ...
    public function setRules($rules)
    {
        return array_merge($rules, [
            ...
            'range' => ['record_status', 'in', 'range' => array_keys(App::keyMapParams('record_status'))],
    ...
    public static function draftAll($condition = '')
    {
        $models = static::findAll($condition);
        $result = static::updateAll(['record_status' => self::RECORD_DRAFT], $condition);
    ...
    public static function deleteAllRow($condition = null, $params = []) 
    {
    ...
    public static function updateAllNoLogs($attributes, $condition = '', $params = [])
    {
        $attributes['updated_at'] = $attributes['updated_at'] ?? App::timestamp();
        $attributes['updated_by'] = $attributes['updated_by'] ?? 0;
    ...
    {
        if (App::isLogin()) {
            $var = App::identity()->can('duplicate', $this->controllerID());
    ...
    {
        if (App::isLogin()) {
            $var = App::identity()->can('delete', $this->controllerID());
    ...
    {
        if (App::isLogin()) {
            $var = App::identity()->can('create', $this->controllerID());
    ...
    {
        if (App::isLogin()) {
            $var = App::identity()->can('view', $this->controllerID());
            if ($var == false) {
                return $var;
            }
        }
    ...
    {
        if (App::isLogin()) {
            $var = App::identity()->can('update', $this->controllerID());
    ...
    public function getTableColumns($tablename=false)
    {
        ...
        $filterColumns = App::identity()->filterColumns($this, false, $tablename);
    ...
    public function getRecordStatusBadge()
    {
        return Html::tag('label', $this->recordStatus['label'], [
            'class' => 'badge badge-' . $this->recordStatus['class']
        ]);
    }

    public function getRecordStatusHtmlWithConfirmation()
    {
        return $this->getRecordStatusHtml(true);
    }

    public function getRecordStatusHtml($withConfirmation=false)
    {
        if (App::isGuest()) {
        ...
        if (($this->beforeCanActivate || $this->beforeCanInactivate)) {
    ...
    public function getCreatedByName()
    {
        if ($this->_createdByName === null) {
            if(($model = $this->createdBy) != null) {
                $this->_createdByName = $model->profile->fullname;
            }
        }
    ...
    {
        if ($this->_updatedByName === null) {
            if(($model = $this->updatedBy) != null) {
                $this->_updatedByName = $model->profile->fullname;
            }
        }

        if ($this->updated_by == 0) {
            $this->_updatedByName = 'None';
        }
    ...
    {
        return App::formatter('asFulldate', $this->updated_at);
    }
