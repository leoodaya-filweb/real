...
$this->title = 'Household: ' . $model->mainAttribute . ' | ' . $step_forms[$step]['title'];
...
<div class="household-view-page">
    ...
    <div class="wizard wizard-2" id="kt_wizard" data-wizard-state="first" data-wizard-clickable="false">
        <div class="wizard-nav flex-lg-shrink-0 w-lg-300px w-xl-375px border-right py-8 px-8 py-lg-20 px-lg-10">
            <?= Html::foreach($step_forms, function($step_form, $key) use($step, $model) {
				$class = ($step == $key)? 'current': (($step > $key)? 'completed': 'pending');
				$url = Html::ifElse($model->no, Url::to(['view', 'no' => $model->no, 'step' => $step_form['slug']]), Url::to(['view', 'step' => $key]));
        ...
        <div class="wizard-body py-0 px-8">
            <div class="row">
                <div class="col">
                    <div class="pb-5" data-wizard-type="step-content" data-wizard-state="current">
                        <?= $this->render('_view/' . $step_forms[$step]['slug'], [
                            'step_form' => $step_forms[$step],
                            'step' => $step,
                            'model' => $model,
                            'head' => $model->familyHead,
                        ]) ?>
                    </div>
