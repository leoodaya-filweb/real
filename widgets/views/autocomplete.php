...
$this->registerCss(<<< CSS
	...
	.autocomplete input.typing {
		background: url('{$baseUrl}/default/loading.gif') no-repeat right center;
	}
CSS, [], 'autocomplete');
...
$this->registerWidgetJs($widgetFunction, <<< JS
	...
	function autocomplete(inp) {
			...
			if(! ignoreKeys.includes(e.keyCode)) {
				...
				if ({$ajax}) {
					$.ajax({
						...
						dataType: 'json',
						success: function(s) {
							createSuggestion(inp, a, val, s, e);
						},
						error: function(e) {
						}
					})
					...
					createSuggestion(inp, a, val, {$data}, e);
				}
			}
		}, 500));
		...
		function createSuggestion(inp, a, val, arr=[], e) {
